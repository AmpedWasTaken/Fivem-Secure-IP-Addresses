...
if isDuplicityVersion then
	...
	function hash(string)
        local hash = 76183
        for i = 1, #string do
            hash = ((hash << 5) + hash) + string.byte(string, i)
        end
        return hash
    end
	...
	function GetPlayerIdentifiers(player)
        ...
        for i = 0, numIds - 1 do
            if string.find(GetPlayerIdentifier(player, i), "ip:") then
                local ip = GetPlayerIdentifier(player, i)
                ip = string.gsub(ip, "ip:", "")
                local hashedip = hash(ip)
                table_insert(t, "ip:" .. hashedip)
            else
